import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class sketch_KHRONOS_SIMPLE extends PApplet {/********************************************************************************************************************************************************************************/
...
float SigmPlateau=sizeX/3.5f, SigmSlope=SigmPlateau/2, SigmRes=sizeX/40;
...
float frameStep=1.5f/numFrames;

// Surface "multiplicative" relaxation factor:
float relaxFactor=0.92f;

// parameters of the physical based model for the water surface:
float water_damp=0.02f;
float temporal_water_level=.26f; // The "water level" is set somewhere in the middle of the video buffer
...
public void draw() { 
  ...
  if (mousePressed&&(mouseButton == LEFT)) pressure=constrain(pressure+6.0f/numFrames, 0,1); // pressure is normalized betweem 0 and 1
  else   pressure=constrain(pressure-2.0f/numFrames, 0, 1);
...
public void drawThings() {
 if (rendering_Mode==RENDERING_FLAT) {
  ...
  if (mousePressed && (mouseButton == RIGHT)) {
  newXmag = (mouseX-width/2)/PApplet.toFloat(width) * TWO_PI; 
  newYmag = (mouseY-height/2)/PApplet.toFloat(height) * TWO_PI; 
  float diff = xmag-newXmag; 
  if (abs(diff) >  0.01f) { xmag -= diff/6.0f; } 
  diff = ymag-newYmag; 
  if (abs(diff) >  0.01f) { ymag -= diff/6.0f; } 
  }
 // println("xmag: " + str(xmag) + " / ymag: "+ str(ymag)+" / newXmag: " + str(newXmag) + " / newYmag: " +str(newYmag));
  ...
  float x=-sizeX/2;
  float y=-sizeY/2;
  float dx=1.0f*(sizeX-1)/(surX-1);
  float dy=1.0f*(sizeY-1)/(surY-1);
...
// -----------------------------------------------------------------------------------------------------------------------------------
public void  surfaceImprinting() { // Updates surface shape from mouse input, only when button LEFT pressed
...
public void resetSurface(int whichSurface, float level) {
 for(int i=0; i<surY; i++) { 
    for(int j=0; j<surX; j++) { 
...
public void simpleMultiplicativeRelax() {
...
if (key == ' ') { 
  ...
  // Control computation grid size (surface): 
   ...
   // Control brush parameters:
    ...
    // Scaling: 
    else if ( keyCode == DOWN ) {scalFactor*=.96f; } 
    else if ( keyCode == UP ) {scalFactor/=.96f; } 
